const NotificationHelper = {
  ...
  async isNotificationReady() {
    ...
    if (!this.isNotificationPermissionGranted()) {
      ...
      if (status === "denied") {
        window.alert(
          "User didn't give the notificationn permission. Another word, notification has been denied by user. If u want to enable it, you can change the notification permission on your browser settings"
        );
        return false;
      }
      if (status === "default") {
